...
describe('RoomFactory', function() {
    ...
    describe('createRoom', function() {
        ...
        it('should create a room', async function() {
            ...
            const tx = await this.roomFactory.createRoom({ from: roomOwner1, value: amount })
            const event = await expectEvent(tx, 'RoomCreated')
        ...
        it('can create multiple rooms', async function () {
            ...
            const tx1 = await this.roomFactory.createRoom({ from: roomOwner1, value: amount })
            const event1 = await expectEvent(tx1, 'RoomCreated')
            const tx2 = await this.roomFactory.createRoom({ from: roomOwner2, value: amount })
            const event2 = await expectEvent(tx2, 'RoomCreated')
            const tx3 = await this.roomFactory.createRoom({ from: roomOwner3, value: 0 })
            const event3 = await expectEvent(tx3, 'RoomCreated')
        ...
        it('can accept an empty deposit', async function () {
            ...
            const tx = await this.roomFactory.createRoom({ from: roomOwner1, value: web3.utils.toWei('0', 'ether') })
            const event = await expectEvent(tx, 'RoomCreated')
